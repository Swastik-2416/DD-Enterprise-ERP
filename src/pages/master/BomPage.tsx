    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <div className="text-xs font-bold uppercase tracking-wider text-slate-500">Configured Recipes</div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-sm text-slate-900">{fg?.name}</span>
                    <span className="text-xs font-mono px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-medium">
                      v{bom.version}.0
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 mt-1">{bom.notes}</p>
          ...
          {selectedBom && (
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-xs">
              <div className="p-5 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-slate-900">{selectedFgItem?.name} Mix Recipe</h3>
                  <p className="text-xs text-slate-500 mt-0.5">Raw material inputs required per 1 unit of finished block</p>
                </div>
                <div className="text-right">
                  <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Base Unit</span>
                  <div className="text-sm font-bold text-slate-800">1 Piece (pc)</div>
                </div>
              </div>
                    ...
                    {lines.map(line => {
                      ...
                      return (
                        <tr key={line.id} className="hover:bg-slate-50/50">
                          <td className="py-3 px-4 font-semibold text-slate-800">{rm?.name}</td>
                          <td className="py-3 px-4 font-mono text-xs text-slate-500">{rm?.sku}</td>
                          ...
                          <td className="py-3 px-4 text-right text-slate-500">{line.wastage_pct}%</td>
                        </tr>
                      )
                    })}
                  </tbody>
              ...
              <div className="p-4 bg-slate-50 border-t border-slate-200 text-xs text-slate-600">
                💡 <span className="font-semibold text-slate-800">Automatic Stock Deduction:</span> When the plant manager confirms a batch in Production Orders, the system queries this formula and automatically subtracts the exact quantity of Cement, Sand, Aggregate, and Fly Ash from raw material warehouse inventory.
              </div>
            </div>
          )}
        </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <h3 className="text-lg font-bold text-slate-900 mb-2">Create Mix Recipe (BOM)</h3>
            <p className="text-sm text-slate-600 mb-4">
              Define the raw materials and proportion required for 1 finished block.
            </p>
            <div className="space-y-3 text-sm">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Finished Good Product</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Notes / Mix Ratio</label>
                <input type="text" placeholder="e.g. 1:1.5:3 concrete mix ratio" className="w-full px-3 py-2 border border-slate-200 rounded-lg" />
              </div>
            </div>
