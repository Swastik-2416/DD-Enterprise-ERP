    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-surface border border-outline-variant rounded-xl p-4">
          <div className="text-xs font-semibold text-outline uppercase tracking-wider">Total Inventory Valuation</div>
          <div className="text-2xl font-bold text-on-surface mt-1">{formatCurrency(totalValuation)}</div>
          <div className="text-xs text-outline mt-1">Valued at standard purchase / base rates</div>
        </div>

        <div className="bg-surface border border-outline-variant rounded-xl p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-outline uppercase tracking-wider">Low Stock Warnings</span>
        ...
        <div className="bg-surface border border-outline-variant rounded-xl p-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-outline uppercase tracking-wider">Total Tracked SKUs</span>
            <div className="h-7 w-7 rounded-lg bg-primary/10 flex items-center justify-center text-primary">
              <Package className="h-4 w-4" />
            </div>
          </div>
          <div className="text-2xl font-bold text-primary mt-2">{mockItems.length} SKUs</div>
      ...
      <div className="bg-surface border border-outline-variant rounded-xl p-4 flex flex-col md:flex-row gap-3 items-center justify-between shadow-xs">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-2 border border-outline-variant rounded-lg text-sm focus:outline-hidden"
          >
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(item => {
                ...
                return (
                  ...
                  >
                    <td className="py-3 px-4 font-mono font-medium text-on-surface-variant">{item.sku}</td>
                    ...
                    <td className="py-3 px-4 text-on-surface-variant">{getCategoryName(item.category_id)}</td>
                    <td className="py-3 px-4 text-right font-bold text-on-surface">
                      {formatNumber(item.qty_on_hand)} <span className="text-xs font-normal text-outline">{unit}</span>
                    </td>
