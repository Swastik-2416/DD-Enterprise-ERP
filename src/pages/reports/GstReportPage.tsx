    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-surface border border-outline-variant rounded-xl p-5 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-outline uppercase tracking-wider">1. Output Tax Liability</span>
            <span className="text-xs bg-indigo-50 text-indigo-700 font-semibold px-2 py-0.5 rounded">GSTR-1</span>
          </div>
          <div className="text-2xl font-bold text-on-surface mt-2">{formatCurrency(totalOutputGst)}</div>
          <div className="text-xs text-outline mt-1">On taxable sales of {formatCurrency(totalSalesTaxable)}</div>

          <div className="mt-4 pt-3 border-t border-slate-100 space-y-1.5 text-xs text-on-surface-variant">
        ...
        <div className="bg-surface border border-outline-variant rounded-xl p-5 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-outline uppercase tracking-wider">2. Input Tax Credit (ITC)</span>
          ...
          <div className="text-xs text-outline mt-1">On taxable inward of {formatCurrency(totalPurchaseTaxable)}</div>
        ...
        <div className="bg-surface border border-outline-variant rounded-xl p-5 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-outline uppercase tracking-wider">3. Net Electronic Cash Ledger</span>
            <span className="text-xs bg-amber-50 text-amber-700 font-semibold px-2 py-0.5 rounded">Form PMT-06</span>
          </div>
          <div className="text-2xl font-bold text-on-surface mt-2">{formatCurrency(netTotalPayable)}</div>
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 space-y-1.5 text-xs text-on-surface-variant">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-on-surface">{formatCurrency(netTotalPayable)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface border border-outline-variant rounded-xl overflow-hidden shadow-xs">
        <div className="px-6 py-4 border-b border-outline-variant bg-background/50 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-on-surface">Outward Supplies Register (Sales / GSTR-1)</h3>
            <p className="text-xs text-outline">All posted tax invoices and registered recipients</p>
          </div>
          <span className="text-xs font-medium text-on-surface-variant font-mono">HSN: 6810 (18% GST)</span>
        </div>
                ...
                <tr key={inv.id} className="hover:bg-background/50">
                  ...
                  <td className="py-3 px-4">
                    <span className={cn(
                      ...
                      inv.type === 'gst' ? 'bg-indigo-100 text-indigo-700' : 'bg-surface-container text-on-surface-variant'
                    )}>
                      {inv.type === 'gst' ? 'B2B GST' : 'Non-GST'}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-right font-medium">{formatCurrency(inv.taxable_amount)}</td>
                  <td className="py-3 px-4 text-right text-on-surface-variant">{formatCurrency(inv.cgst_amount)}</td>
                  <td className="py-3 px-4 text-right text-on-surface-variant">{formatCurrency(inv.sgst_amount)}</td>
                  <td className="py-3 px-4 text-right font-bold text-on-surface">{formatCurrency(inv.total_amount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-surface border border-outline-variant rounded-xl overflow-hidden shadow-xs">
        <div className="px-6 py-4 border-b border-outline-variant bg-background/50 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-on-surface">Inward Supplies Eligible for ITC (Purchases / GSTR-3B)</h3>
            <p className="text-xs text-outline">Eligible input tax credit on raw materials (Cement 28%, Aggregate 5%, Sand 5%)</p>
          </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {postedPurchases.map(inv => (
                <tr key={inv.id} className="hover:bg-background/50">
                  <td className="py-3 px-4 font-mono font-medium text-primary">{inv.invoice_number}</td>
                  <td className="py-3 px-4 text-on-surface-variant">{inv.date}</td>
                  <td className="py-3 px-4 font-mono text-xs text-outline">{inv.supplier_invoice_number || '—'}</td>
                  <td className="py-3 px-4 text-right font-medium">{formatCurrency(inv.taxable_amount)}</td>
